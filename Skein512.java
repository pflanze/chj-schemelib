...
public class Skein512 {
    ...
    private void startNewType(long type) {
        tweak0 = 0; tweak1 = T1_FLAG_FIRST | type;
    }
    ...
    private void processBlock(byte[] block, int off, int blocks, int bytes) {
        while (blocks-- > 0) {
            ...
            int[] mod3 = MOD3;
            int[] mod9 = MOD9;
            ts[3] = ts[0] = tweak0; ts[4] = ts[1] = tweak1;
            ts[2] = tweak0 ^ tweak1;
            long[] c = x;
            long[] ks = keySchedule;
            ...
            for (int r = 1; r <= ROUNDS / 4; r += 2) {
                ...
                x1 = rotlXor(x1, R00, x0 += x1); x3 = rotlXor(x3, R01, x2 += x3);
                x5 = rotlXor(x5, R02, x4 += x5); x7 = rotlXor(x7, R03, x6 += x7);
                x1 = rotlXor(x1, R10, x2 += x1); x7 = rotlXor(x7, R11, x4 += x7);
                x5 = rotlXor(x5, R12, x6 += x5); x3 = rotlXor(x3, R13, x0 += x3);
                x1 = rotlXor(x1, R20, x4 += x1); x3 = rotlXor(x3, R21, x6 += x3);
                x5 = rotlXor(x5, R22, x0 += x5); x7 = rotlXor(x7, R23, x2 += x7);
                ...
                x1 = rotlXor(x1, R50, x2 += x1); x7 = rotlXor(x7, R51, x4 += x7);
                x5 = rotlXor(x5, R52, x6 += x5); x3 = rotlXor(x3, R53, x0 += x3);
                x1 = rotlXor(x1, R60, x4 += x1); x3 = rotlXor(x3, R61, x6 += x3);
                x5 = rotlXor(x5, R62, x0 += x5); x7 = rotlXor(x7, R63, x2 += x7);
