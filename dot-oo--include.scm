...
(define-typed (dot-oo:method-type-maybe-ref-method #(vector? vec) #(fxnatural? n) obj)
  (let ((end (-> fxpositive? (fx+ n n))))
    (assert (fx< end (fx- (##vector-length vec) n)))
    (let ()
      (declare (not safe))
      (let lp ((i n))
	(if (fx< i end)
...
(define (dot-oo:method-table-maybe-ref-method tbl obj)
  (let* ((vec (unbox tbl))
	 (n (fxarithmetic-shift (vector-length vec) -2)))
