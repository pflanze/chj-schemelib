...
(define-macro (define/check->integer c-name name1 name2 type-argname-alist returntype)
  (or (string=? c-name
		(string-strip-until-last-chars (symbol->string name1) ":_")
		(string-strip-until-last-chars (symbol->string name2) ":"))
      (warn "not consistent: " c-name name1 name2))
  (case returntype
    ((int uid_t gid_t
	  ssize_t;; OK?todo.
	  )
     (let ((argnames (map cadr type-argname-alist))
	  ...
	  (error-to-posix-exception
	   ((c-lambda ,argtypes
	    ...
	    ,@argnames)))))
    (else (error "unknown return type:" returntype))))
