...
(require cj-env	;; warn, for debgging  ah and now thunk
		 ...
		 (if (symbol? key)
			 ...
			 (cond ((pair? val)
				...
				(if xml?
			       ...
			       (else
				(atom>> val)))))
		     (error "attribute key is not a symbol:" key) ;; or treat the whole thing like elements, e.g. accept listwraps?
		     ))
	       ;; else something is strange.
	       ;;(error "attrlist-display: invalid attrlist:" attrlist)))
	       ;; nope, accept it, like in  '(img (@ (src "http://www.ethlife.ethz.ch/pix/favicon.png") (align "absbottom") (alt "") #f (border 0)))
	       ))
...
(define (@string>>htmlquoted str port count-chars? in-attributes?)
  (let ((len (##string-length str)))
    (let loop ((pos 0))
      (if (##fixnum.< pos len)
	  (begin
	    (@char>>htmlquoted (##string-ref str pos) port count-chars? in-attributes?)
	    (loop (##fixnum.+ pos 1)))))))
...
(define (atom-or-list>>htmlquoted value port count-chars? xml? in-attributes? maybe-level)
  ;;(since this is used only once, I could just as well integrate it into sxml>>fast)
  (define subproc
    (lambda(item)
...
(define (atom>>htmlquoted atom port count-chars? xml? in-attributes?)
  (let self ((atom atom))
    (cond ((string? atom)
	  ...
	  ((symbol? atom)
	   (@symbol>> atom port) ;;; I'm using this for outputting |&nbsp;| etc.; but maybe should use something else, for better error checking.(flattenedtags)  ?
					; 	 (serializeprocess-string-reset!)
	   )
	  ((null? atom)
	   #|nothing|#)
    ...
    (let ((content (cond ((@sxml-attributes l) ;; well.. what should happen with *COMMENT* elements having attributes? being handled strangely currently, probably.
	  ...
	  (let* ((maybe-indent>> (lambda (maybe-level)
	    ...
	    (cond ((pair? content)
		  ...
		  (else
		   ;; (error "@sxml-element>>: improper list: " l)
		   (out>>/body>> (& (sxml>>fast content port xml? next-level)))	;; (output a single element as opposed to a list) --todo:  should just use a for-each which accepts dotted lists.
...
(define *warn-thunk-evaluation* #f)
...
;;FRAGE warum wenn ich doch atom schon habe, nicht die promise, null, procedure, [und sogar pair?] ebenfalls dort drin?
;;[  hm war es zur unterscheidung von  rein rekursen vs andere ? (beim ein fügen von maybe-level merk isch dass das ev schönisch? ?)]
...
; (define (sxml-element:add-attributes-unless-present element alis)
;   (define (process maybe-attrs)
;     (warn "maybe-attrs=" maybe-attrs))
;   (with-sxml-element-attributes/else ;;d'oh  did not bring anything: (@) is understood as no.
;    element
;    process
;    (lambda ()
;      (process #f))))
  ...
  (let ((lang (or (sxml-element-attribute-ref item 'xml:lang #f)
    ...
    (sxml>>fast (sxml-element:add-attributes-unless-present
		 ...
		 `((xmlns "http://www.w3.org/1999/xhtml")
		   (xml:lang ,lang)
		   (lang ,lang)	;; (note: would be added (copied from xml:lang) by xmllint anyway if missing. but be kind here already)
		   ))
		port
		#t
		(and maybe-level
  ...
  (lambda (item outpath #!optional noblanks)
	...
	(with-exception-catcher
	 ...
	 (thunk
	  ...
	  (close-output-port p)
	  ;;(thread-sleep! 0.03)  much faster than process-status but of course unsafe in every regard
	  (let ((r (process-status p) ;; sloooow.  aha: see _io.scm ##thread-sleep! loop / polling notice
		   ))
	    (if (= r 0)
